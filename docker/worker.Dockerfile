...
COPY --from=build --chown=appuser:appuser /app/apps/worker/dist ./apps/worker/dist
COPY --from=build --chown=appuser:appuser /app/apps/worker/package.json ./apps/worker/package.json
